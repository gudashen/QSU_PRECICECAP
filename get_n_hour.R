...
directory <- '/oak/stanford/groups/zihuai/precicecap_data_main/'
full_time <- TRUE
time_window <- FALSE
time_break <- 5
clinic_file <- '/oak/stanford/groups/zihuai/precicecap_data_main/Clinic Data/Output/baseline_CRF.csv'
## search all files with an extension ".h5" under the directory, including sub-folders
subject_files <- list.files(directory, recursive = T, pattern = '*.h5')   
full_path <- paste0(directory, subject_files)
clinic <- read_csv(clinic_file)    ## load clinical data

path <- full_path[job_index]


get_n_min_avg <- function(path) {
     if (full_time == T) {    ## Below is the code for extracting complete non-eeg data for each patient
          if (!missing(time_window)) {
               stop('argument "full_time" must be set to "FALSE" when "time_window" has a value')
                    ...
                    aggr_data <- foreach(a = path, .combine = plyr::rbind.fill,     ## level 1 foreach loop to search over the HDF5 file(s) for each patient
                                    .packages = c('stringi', 'plyr', 'dplyr', 'rhdf5', 
                                                  'tidyr', 'readr', 'lubridate', 'doParallel')) %:% 
                         foreach(b = iter(h5ls(a, datasetinfo = F) %>%    ## level 2 foreach loop to return the field names of non-eeg forms/trends and of their timestamps under each patient, then iterating each pair of names by row.
                                          filter(group %in% c('/numerics', '/waves') & 
                                                      !grepl('Timestamps|Quality', 
                                                             name, 
                                                             ignore.case = T)) %>% 
                                          select(1:2) %>% 
                                 ...
                                 .combine = comb) %dopar% {
                                      ...
                                      field <- tryCatch(h5read(a, b[, 2], bit64conversion='bit64'), 
                                                        error = function(err) NULL)    ## read each non-eeg waveform/trend
                                      time <- as.POSIXct(tryCatch(h5read(a, b[, 3], bit64conversion='bit64'), 
                                                                  error = function(err) NULL) / 1e6, 
                                                         origin = '1970-01-01', tz = 'GMT')    ## read each timestamp
                                      full_field <- aggregate(field, 
                                                              list(cut(time[1:length(field)], 
                                                                              breaks = paste0(
                                                                                   time_break, ' min'))
                                                                   ), 
                                                              mean)[, 'x']    ## calculate moving averages for each xx-min interval
                                      full_window <- aggregate(time[1:length(field)], 
                                                               list(cut(time[1:length(field)], 
                                                                        breaks = paste0(
                                                                             time_break, ' min'))
                                                                    ), 
                                                               first)[, 'x']    ## get start time for each xx-min interval
                                      means_time <- data.frame(full_field, full_window)
                                      colnames(means_time)[1:2] <- c(b[, 1], 
                                                                     paste0(b[, 1], '_start'))
                                      EMS <- clinic[clinic$zSubjectID == subject_num, 
                                                    c('F502Q02', 'F502Q03', 'F502Q06', 'F502Q07')][1,]    ## get EMS info from clinical dataset for each patient
                                      call_time <- as.POSIXct(paste(EMS$F502Q02, EMS$F502Q03), 
                                                              format = "%d%b%Y %H:%M:%S",
                                                              tz = case_when(
                                                                   site_num %in% c('1753') ~ 
                                                                        'America/Chicago',
                                                                   site_num %in% c('1125', '1435') ~ 
                                                                        'America/Los_Angeles', 
                                                                   T ~ 'America/New_York'))
                                      arrival_time <- as.POSIXct(paste(EMS$F502Q06, EMS$F502Q07), 
                                                                 format = "%d%b%Y %H:%M:%S",
                                                                 tz = case_when(
                                                                      site_num %in% c('1753') ~ 
                                                                           'America/Chicago', 
               ...
               if (missing(time_window)) {
                    stop('argument "time_window" must have a value when "full_time" is set to "FALSE"')
                         ...
                         aggr_data <- foreach(a = path, .combine = plyr::rbind.fill, 
                                              .packages = c('stringi', 'plyr', 'dplyr', 'rhdf5',
                                                            'tidyr', 'readr', 'lubridate', 'doParallel')) %:% 
                              foreach(b = iter(h5ls(a, datasetinfo = F) %>% 
                                      ...
                                      .combine = cbind) %dopar% {
                                           ...
                                           means_time <- data.frame(aggregate(field[1:length(n_hr_window)],    ## calculate moving averages for each xx-min interval
                                                                              list(cut(n_hr_window, 
                                                                                       breaks = paste0(time_break, ' min'))),
                                           ...
                                           EMS <- clinic[clinic$zSubjectID == subject_num, 
                                                         c('F502Q02', 'F502Q03', 'F502Q06', 'F502Q07')][1, ]
                                           call_time <- as.POSIXct(paste(EMS$F502Q02, EMS$F502Q03), 
                                                                   format = "%d%b%Y %H:%M:%S",
                                                                   tz = case_when(site_num %in% c('1753') 
                                                                                  ~ 'America/Chicago', 
                                                                                  site_num %in% c('1125', '1435') 
                                                                                  ~ 'America/Los_Angeles', 
                                                                                  T ~ 'America/New_York'))
